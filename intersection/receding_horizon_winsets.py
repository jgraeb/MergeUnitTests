...
def specs_car_rh():
    ...
    ego_vars['y'] = (0,7)
    ego_vars['z'] = (0,7)
    ego_init = {'y='+str(1), 'z='+str(1)}
    ego_safe = get_ego_safety()
    ego_prog = {'z=2'}
    ...
    test_vars['z1'] = (0,7)
    test_vars['z2'] = (0,7)
    test_init = {'y1='+str(0), 'y2='+str(0), 'z1='+str(7), 'z2='+str(7)}
    test_safe = set()
    test_safe = get_test_safety()
    test_safe = add_goal_specs(test_safe)
    test_prog = set()
    # test_prog = add_psi_j_progress(tracklength, j, merge_setting)

    ego_n = ... # Fill in number of ego states
    tester_n = ... # Fill in number of tester states
    G, st2ver_dict, ver2st_dict, state_tracker, state_dict_test, state_dict_system = get_all_states(ego_n, tester_n)


    goal_lambda = construct_lambda_function()
...
def get_winset_rh(Vij, state_tracker, ver2st_dict,ego_spec_orginal, test_spec_orginal, state_test_dict, state_system_dict, G):
    ...
    for j in np.linspace(jmax, 0, jmax+1):
        if j%2 == 0:
            ...
            states_in_fp, states_out_fp = check_all_states_in_fp(agentlist, w_set, fp, aut)
            ...
            states_in_winset, states_out_winset = check_all_states_in_winset_rh(agentlist, w_set, fp, aut, state_test_dict, state_system_dict, goal_states, G, ver2st_dict, start_set)
            Wij.update({j: states_in_winset})
    # st()
    return Wij

def get_tester_states_in_winsets():
    """
    ...
    """
    ego_spec, test_spec, Vij_dict, state_tracker, ver2st_dict, G, state_dict_test, state_dict_system = specs_car_rh()
    Wij = dict()
    # Go through all the goal states
    # st()
    for key in Vij_dict.keys():
        Wj = get_winset_rh(Vij_dict[key], state_tracker, ver2st_dict,ego_spec, test_spec, state_dict_test, state_dict_system, G)
        Wij.update({key: Wj})
    # st()
...
def check_system_states_in_winset(origin_state, state, ver2st_dict, state_tracker, Wij, debug = False):
    '''
    Function to check if system states are in the winning set.
    '''
...
def find_next_states(state, ver2st_dict, state_tracker):
    ...
    x = state['y']
    y = state['z']
    tester_states = [(state['y1'],state['z1']),(state['y2'],state['z2'])]
...
def get_dict_inv_multiple(dict_in, value):
    '''
    Get the dictionary representation of state and getting the key value in terms of state
    '''
    keys = []
